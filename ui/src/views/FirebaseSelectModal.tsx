    ...
    public render() {
        const getOptions = () => this.props.options.map((option) => ([
            ...
            <div class='SelectModal__cell SelectModal__cell--action'>
                <button style="background-color: #49B04D" id="sharebutton" class="buttonMenu icon-share"
                        onClick={() => this.props.onShare(option)}>{navLabels[11]}</button>
                <button class="buttonMenu error" id="deletebutton"
                        onClick={() => this.props.onDelete(option)}>{generic[4]}</button>
                    ...
                    <footer class='SelectModal__buttons'>
                        {
                            this.getButtons().map((button) => (
                                <button style={{
                                    ...
                                    [`margin-${button.position === 'left' ? 'right' : 'left'}`]: '8px'
                                }} onClick={() => this.props.onButtonClick(button.key)}>{generic[3]}</button>
                            ))
                        }
                    </footer>
