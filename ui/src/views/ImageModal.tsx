    ...
    public render() {
        const getOptions = () => (
            this.props.options.map((option) => (
                <div className='ImageModalOption' onClick={() => this.props.onSelect(option)}
                     style="position: relative; cursor: pointer;">
                    <img src={option.image}/>
                    <div className='ImageModalOptionButtonContainer'>
                        <button className="ImageSelectButton"
                                onClick={() => this.props.onSelect(option)}>{option.title}</button>
                        <div className='ImageHelpIconContainer' style="text-align: center;">
                            <a class='icon-help-circled' style='color: rgba(255, 255, 255, 0.8);font-size: 23px;'
                               href={option.help}></a>
                        </div>
                    </div>
                </div>
